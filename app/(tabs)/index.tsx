  ...
  return <View style={{flex:1,backgroundColor:colors.bg,paddingTop:insets.top}}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:32}}>
      ...
      <View style={{paddingHorizontal:spacing.lg,paddingTop:spacing.md,paddingBottom:spacing.sm}}>
        <Text style={[type.label,{marginBottom:3}]}>{['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][TODAY.getDay()]} · {todayLbl}</Text>
        <Text style={{fontFamily:fonts.serifItalic,fontSize:30,lineHeight:40,color:colors.text}}>Good evening, Cody</Text>
      </View>
      ...
      <View style={{marginTop:spacing.lg}}>
        ...
        {ACTIVITY.slice(0,3).map(item=>{
          ...
          return <View key={item.id} style={{flexDirection:'row',gap:12,marginHorizontal:spacing.lg,marginBottom:8,backgroundColor:colors.surface,borderRadius:radius.lg,padding:14,...shadow.soft}}>
            {bookObj&&item.type!=='milestone'
              ?<TouchableOpacity onPress={()=>setDetailId(bookObj.id)} style={{flexShrink:0}}>
      ...
      {recs.length>0&&<View style={{marginTop:spacing.lg}}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:spacing.lg,gap:10}}>
          {recs.slice(0,6).map(r=>{const b=all.find(x=>x.id===r.id);const seed=all.find(x=>x.id===r.reasonId);if(!b)return null;
            return <TouchableOpacity key={r.id} onPress={()=>setDetailId(r.id)} activeOpacity={0.85}
              ...
              <Text style={{fontFamily:fonts.serifBold,fontSize:12,color:colors.text,marginTop:8,lineHeight:16}} numberOfLines={2}>{b.title}</Text>
              <Text style={{fontFamily:fonts.sans,fontSize:10,color:colors.text3,marginTop:2}} numberOfLines={1}>{b.author}</Text>
              {seed&&<Text style={{fontFamily:fonts.sansMedium,fontSize:9,color:colors.accent,marginTop:6,lineHeight:13}} numberOfLines={2}>Because you loved {seed.title}</Text>}
            </TouchableOpacity>;})}
        </ScrollView>
      </View>}
      ...
      <View style={{flexDirection:'row',gap:spacing.sm,marginHorizontal:spacing.lg,marginTop:spacing.lg}}>
        {/* Goal */}
        <View style={{flex:1,backgroundColor:pastels.sage,borderRadius:radius.lg,padding:spacing.md}}>
        ...
        {/* Featured challenge */}
        {featuredChallenge&&<View style={{flex:1.4,backgroundColor:pastels.lavender,borderRadius:radius.lg,padding:spacing.md}}>
