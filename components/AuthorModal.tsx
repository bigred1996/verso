  ...
  return <Modal visible={!!author} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
    <SafeAreaView style={{flex:1,backgroundColor:colors.bg}}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Photo area */}
        <View style={{height:260,backgroundColor:colors.surface2,alignItems:'center',justifyContent:'center'}}>
          {photo
            ? <Image source={{uri:photo}} style={StyleSheet.absoluteFill} resizeMode="cover"/>
            : <Text style={{fontFamily:fonts.serifItalic,fontSize:80,color:colors.text3}}>{initials}</Text>}
          <View style={[StyleSheet.absoluteFill,{backgroundColor:colors.bg,opacity:0.0}]} pointerEvents="none"/>
          {/* bottom fade */}
          <View style={{position:'absolute',left:0,right:0,bottom:0,height:90,backgroundColor:colors.bg,opacity:0.55}} pointerEvents="none"/>
        </View>
        <View style={{padding:spacing.lg}}>
          <Text style={{fontFamily:fonts.serifItalic,fontSize:26,color:colors.text,marginBottom:4}}>{author}</Text>
          {a&&<Text style={{fontFamily:fonts.sans,fontSize:11,color:colors.text3,letterSpacing:1.2,textTransform:'uppercase',marginBottom:10}}>{a.born}</Text>}
          {a&&<View style={{flexDirection:'row',flexWrap:'wrap',gap:5,marginBottom:16}}>
            {a.awards.map(aw=><View key={aw} style={{paddingHorizontal:8,paddingVertical:3,borderWidth:1,borderColor:'rgba(61,107,72,0.4)'}}>
              <Text style={{fontFamily:fonts.sansBold,fontSize:9,color:colors.accent,letterSpacing:0.6,textTransform:'uppercase'}}>{aw}</Text>
            </View>)}
          </View>}
          {a
            ? <Text style={{fontFamily:fonts.sans,fontSize:14,color:colors.text2,lineHeight:25,marginBottom:22}}>{a.bio}</Text>
            : <Text style={{fontFamily:fonts.sans,fontSize:13,color:colors.text3,marginBottom:22}}>Author profile coming soon.</Text>}
          ...
          {books.map(b=><TouchableOpacity key={b.id} onPress={()=>onOpenBook(b.id)}
            style={{flexDirection:'row',gap:12,paddingVertical:12,borderBottomWidth:1,borderBottomColor:colors.border,alignItems:'flex-start'}}>
